...
KC_DOWN,  KC_LEFT,            KC_SPC,   TD_BCK,   TT_SYM,   KC_MUTE,      KC_MPLY,  TT_MOU,   KC_ENT,   KC_LSFT,            KC_RGHT,  KC_UP
),
...
KC_TRNS,  KC_TRNS,            KC_TRNS,  KC_TRNS,  KC_TRNS,  KC_TRNS,      KC_TRNS,  TD_MOU,   KC_TRNS,  KC_TRNS,            KC_TRNS,  KC_TRNS
),
...
KC_TRNS,  KC_TRNS,            KC_TRNS,  KC_TRNS,  TD_SYM,   KC_TRNS,      KC_TRNS,  TD_QWE,   KC_TRNS,  KC_TRNS,            KC_TRNS,  KC_TRNS
) 
...
//  Layer         - Left Rotary Encoder             - Left Scroll Encoder             - Right Scroll Encoder            - Right Rotary Encoder
    [_QWERTY] = { ENCODER_CCW_CW(KC_VOLU, KC_VOLD), ENCODER_CCW_CW(KC_WH_U, KC_WH_D), ENCODER_CCW_CW(KC_WH_R, KC_WH_L), ENCODER_CCW_CW(KC_MNXT, KC_MPRV) },
    [_SYMBOL] = { ENCODER_CCW_CW(KC_VOLU, KC_VOLD), ENCODER_CCW_CW(KC_WH_U, KC_WH_D), ENCODER_CCW_CW(KC_WH_R, KC_WH_L), ENCODER_CCW_CW(KC_MNXT, KC_MPRV) },
    [_MOUSE]  = { ENCODER_CCW_CW(KC_VOLU, KC_VOLD), ENCODER_CCW_CW(KC_WH_U, KC_WH_D), ENCODER_CCW_CW(KC_WH_R, KC_WH_L), ENCODER_CCW_CW(KC_MNXT, KC_MPRV) },
    #ifdef JOYSTICK_ENABLE
    [_GAMES]  = { ENCODER_CCW_CW(KC_VOLU, KC_VOLD), ENCODER_CCW_CW(KC_WH_U, KC_WH_D), ENCODER_CCW_CW(KC_WH_R, KC_WH_L), ENCODER_CCW_CW(KC_MNXT, KC_MPRV) },
    #endif
    #ifdef STENO_ENABLE
    [_STENO]  = { ENCODER_CCW_CW(KC_VOLU, KC_VOLD), ENCODER_CCW_CW(KC_WH_U, KC_WH_D), ENCODER_CCW_CW(KC_WH_R, KC_WH_L), ENCODER_CCW_CW(KC_MNXT, KC_MPRV) },
    #endif
};
#endif
...
bool encoder_update_kb(uint8_t index, bool clockwise) {
    ...
    if (index == 0) { /* LEFT ROTARY ENCODER */
        if (clockwise) { /* Volume Down */
            tap_code(KC_VOLD);
        } else { /* Volume Up */
            tap_code(KC_VOLU);
        }
    } else if (index == 1) { /* LEFT SCROLL ENCODER */
        if (clockwise) { /* Mouse wheel down */
            tap_code(KC_WH_D);
        } else { /* Mouse wheel up */
            tap_code(KC_WH_U);
        }
    } else if (index == 2) { /* RIGHT SCROLL ENCODER */
        if (clockwise) { /* Mouse wheel left */
            tap_code(KC_WH_L);
        } else { /* Mouse wheel right */
            tap_code(KC_WH_R);
        }
    } else if (index == 3) { /* RIGHT ROTARY ENCODER */
        if (clockwise) { /* Browser tab left */
            register_code(KC_LCTL);
            tap_code(KC_PGUP);
            unregister_code(KC_LCTL);
        } else { /* Browser tab right */
            register_code(KC_LCTL);
            tap_code(KC_PGDN);
            unregister_code(KC_LCTL);
        }
